 ...
 */
function generatepress_child_cached_dns_lookup($hostname) {
    $transient_key = 'generatepress_child_dns_' . preg_replace('/[^a-z0-9_]/i', '_', $hostname);
 ...
 * - Running on localhost/127.0.0.1/::1
 * - Running on common local dev domains (.local, .test, .dev, .localhost)
 * - Hostname resolves to 127.0.0.1 (IPv4 only; gethostbyname() doesn't support IPv6)
...
function generatepress_child_is_dev_environment() {
    ...
    // Use trim() instead of sanitize_text_field() to preserve valid hostnames/IPs
    ...
    if ($server_name && !str_contains($server_name, ':')) {
        $resolved_ip = generatepress_child_cached_dns_lookup($server_name);
    ...
    if ($http_host_clean && $http_host_clean !== $server_name && !str_contains($http_host_clean, ':') && !str_starts_with($http_host_clean, '[')) {
        $resolved_ip = generatepress_child_cached_dns_lookup($http_host_clean);
        // If resolution succeeded and points to localhost
